...
function install() {
  ...
  function DevTools() {
    ...
    const [persist, setPersist] = useLocalStorageState('__cheatsheep_devtools_persist__', false)
    const show = persist || hovering
    const [tabIndex, setTabIndex] = useLocalStorageState('__cheatsheep_devtools_tab_index__', 0)
    const togglePersist = () => {
      setPersist((v) => !v)
    }
...
function FailureRate() {
  const [failureRate, setFailureRate] = useLocalStorageState('__cheatsheep_failure_rate__', 0)
...
function RequestMinTime() {
  const [minTime, setMinTime] = useLocalStorageState('__cheatsheep_min_request_time__', 400)
...
function RequestVarTime() {
  const [varTime, setVarTime] = useLocalStorageState('__cheatsheep_variable_request_time__', 400)
...
function RequestFailUI() {
  ...
  const [failConfig, setFailConfig] = useLocalStorageState<{requestMethod: string; urlMatch: string}[]>(
    '__cheatsheep_request_fail_config__',
    []
  )
